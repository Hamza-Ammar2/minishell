...
static void	free_str_args(char **args, char *str, char *cmd)
{
	if (str != cmd)
		free(str);
	free_splits(args);
...
int	exit_exec(t_shell *shell, t_command *cmds, char **args, char *str)
{
	...
	if (!str)
		(free_splits(args), ft_fprintf(2, "command not found\n"),
			exit_nice(shell, cmds, 127));
	if (S_ISDIR(st.st_mode))
		(free_str_args(args, str, cmd), ft_fprintf(2, "Is a directory\n"),
			exit_nice(shell, cmds, 126));
	if (access(str, X_OK) != 0)
		(free_str_args(args, str, cmd), ft_fprintf(2, "Permission denied\n"),
			exit_nice(shell, cmds, 126));
	return (0);
}
